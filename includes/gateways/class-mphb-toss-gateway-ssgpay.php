...
/**
 * 토스페이먼츠 쓱페이(SSG Pay) 게이트웨이 클래스입니다.
...
class TossGatewaySsgpay extends TossGatewayBase {
     ...
     * 쓱페이와 같은 간편결제는 'CARD'로 처리되고, JS에서 easyPay provider 코드가 사용됩니다.
    ...
    /**
     * 간편결제 제공사 코드를 반환합니다.
     * @return string 간편결제 제공사 코드 (쓱페이)
     */
    public function getEasyPayProviderCode(): string {
        return 'SSG'; // 토스페이먼츠 문서 기준 SSGPAY 또는 SSG 일 수 있음, 확인 필요
    }

    /**
     * 선호하는 결제 흐름 모드를 반환합니다.
     * 쓱페이는 'DIRECT' (직접 연동) 방식을 사용합니다.
     ...
     * 부모 클래스의 처리를 호출한 후, 쓱페이 관련 정보를 결제 메타데이터로 저장합니다.

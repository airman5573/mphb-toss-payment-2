...
class TossGatewayCard extends TossGatewayBase {
     ...
     * 부모 클래스의 처리를 호출한 후, 카드 결제 관련 정보를 결제 메타데이터로 저장합니다.
     * @param Payment $payment 결제 객체
     * @param Booking $booking 예약 객체
     * @param object $tossResult 토스페이먼츠 API 응답 객체
     */
    protected function afterPaymentConfirmation(Payment $payment, Booking $booking, $tossResult) {
        parent::afterPaymentConfirmation($payment, $booking, $tossResult); // 부모 클래스 처리 (가상계좌 등 공통 로직)
        ...
        if (isset($tossResult->card)) {
            ...
            update_post_meta($payment->getId(), '_mphb_toss_card_installment_plan_months', $cardInfo->installmentPlanMonths ?? 0); // 할부 개월 수
            update_post_meta($payment->getId(), '_mphb_toss_card_approve_no', $cardInfo->approveNo ?? ''); // 승인 번호
            update_post_meta($payment->getId(), '_mphb_toss_card_type', $cardInfo->cardType ?? ''); // 카드 타입 (신용, 체크 등)
            update_post_meta($payment->getId(), '_mphb_toss_card_owner_type', $cardInfo->ownerType ?? ''); // 소유자 타입 (개인, 법인)
        } else { // card 객체가 없는 경우

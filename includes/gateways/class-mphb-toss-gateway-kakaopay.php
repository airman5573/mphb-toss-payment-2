...
class TossGatewayKakaopay extends TossGatewayBase {
     ...
     * 카카오페이와 같은 간편결제는 'CARD' (카드)로 처리되고, JS에서 easyPay provider 코드가 사용됩니다.
    ...
    /**
     * 간편결제 제공사 코드를 반환합니다.
     * @return string 간편결제 제공사 코드 (카카오페이)
     */
     ...
     * 부모 클래스의 처리를 호출한 후, 카카오페이 관련 정보를 결제 메타데이터로 저장합니다.
    ...
    protected function afterPaymentConfirmation(Payment $payment, Booking $booking, $tossResult) {
        ...
        // 토스 API 응답에 easyPay 정보가 있는 경우 (일반적인 간편결제 응답)
        if (isset($tossResult->easyPay)) {
            ...
            // 카드 정보를 카카오페이 정보로 간주하고 저장 (회사명, 마스킹된 번호 등)
